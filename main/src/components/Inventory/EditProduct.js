import { Dialog, Grid, TextField, Typography, Button, Chip, Tooltip, IconButton, Select, MenuItem, ListItemText, Checkbox } from "@mui/material";
...
function EditDialog(props) {
    ...
    useEffect(() => {
        if (item.tag != undefined)
            setEditProductTags(item.tag);
        if (item.category != undefined)
            setEditProductCategories(item.category);
    }, [])
                    ...
                    <Grid container spacing={5} >
                        ...
                        <Grid item xs={4}>
                            ...
                            {editProductTags.map(tag => {
                                return (<Chip label={tag} sx={{ margin: "1%" }} onDelete={() => { handleTagDelete(tag) }}></Chip>)
                            })}
                            <Typography>
                                Add Tag:
                            </Typography>
                            <TextField
                                value={curTag}
                                onChange={(event) => { setCurTag(event.target.value) }}
                            >
                            </TextField>
